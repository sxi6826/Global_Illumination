...
def phong(xi, yi, zi, xc, yc, zc, e1, e2, shape, depth = 0, lx=0.0,ly=0.0,lz=0.0):
    ...
    if (depth < 1):
        ...
        if (kt > 0):
            tx, ty,tz = transmit(s[0,0], s[0,1], s[0,2], n[0,0], n[0,1], n[0,2])
            t[0,0], t[0,1], t[0,2] = tx,ty,tz
            magt = np.linalg.norm(t)
            t[0, 0], t[0, 1], t[0, 2] = t[0, 0] / magt, t[0, 1] / magt, t[0, 2] / magt

            sh_pe, xip, yip, zip, e1n, e2n = ray_intersection(t[0, 0], t[0, 1], t[0, 2], xi, yi, zi)
            if (sh_pe == 'bs'):
                clr1 = phong(xip, yip, zip, 0.0, 1.6, 5.0, 0, 0, 's', depth + 1, xi, yi, zi)
            elif (sh_pe == 'ss'):
                clr1 = phong(xip, yip, zip, 2.5, -0.5, 6.0, 0, 0, 's', depth + 1, xi, yi, zi)
                ...
                clr1 = (0, 0, 0)
            tll[0, 0], tll[0, 1], tll[0, 2] = clr1[0], clr1[1], clr1[2]

    if (kd[0, 0] + ks[0, 0] + kr*ill[0,0]  < 0):
        kd[0, 0] = 0
        ks[0, 0] = 0
        ill[0, 0] = 0
    if (kd[0, 1] + ks[0, 1] + kr*ill[0,1] < 0):
        kd[0, 1] = 0
        ks[0, 1] = 0
        ill[0, 1] = 0
    if (kd[0, 2] + ks[0, 2] + kr*ill[0,2] < 0):
        kd[0, 2] = 0
        ks[0, 2] = 0
        ill[0, 2] = 0


    clr = ((kd[0, 0] + ks[0, 0] + (kr*ill[0,0]) + (kt*tll[0,0])), (kd[0, 1] + ks[0, 1] + (kr*ill[0,1])
                                                                   + (kt*tll[0,1])), (kd[0, 2] + ks[0, 2] + (kr*ill[0,2]) + (kt*tll[0,2])))
    return clr


def transmit(sx,sy,sz,nx,ny,nz):

    s = np.matrix('0.0,0.0,0.0')
    s[0, 0], s[0, 1], s[0, 2] = sx, sy, sz

    n = np.matrix('0.0,0.0,0.0')
    n[0, 0], n[0, 1], n[0, 2] = nx, ny, nz
    nt = n.transpose()
