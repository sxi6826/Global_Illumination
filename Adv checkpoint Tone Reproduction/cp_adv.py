...
def tonerep(lst, H, W):
    ...
    for x in range(int(H)):
        ...
        for y in range(int(W)):
            ...
            #if(ill!=0.06 and ill!= 0.67 and ill!= 0.27 and ill!=0.94 and ill!= 0.33 and ill!= 0.73 and ill!= 0.0):
                #print (ill)
            if(lwmax < ill):
                lwmax = ill
            slst.append(ill)
    ...
    for x in range(int(H)):
        ...
        for y in range(int(W)):
            lw1 = l[x][y]
            #lw = lw1/lwa
            num = log(lw1+1)/log(10)
            lm = (lw1/lwmax)**lg
            lm = lm*8
            denom2 = log(lm+2)/log(10)
            ld = num/(denom1*denom2)
            #print (ld)
            if(lw1 != 0):
                r = ld*(int(lst[x][y][0]))/lw1
                g = ld*(int(lst[x][y][1]))/lw1
                b = ld*(int(lst[x][y][2]))/lw1
            else:
                r,g,b = int(lst[x][y][0]), int(lst[x][y][1]), int(lst[x][y][2])
            temp=(r,g,b)
            #print temp
            slst.append(temp)
        lst1.append(slst)
